...
class PeoteSocket {
	...
	{
		var supported:Bool = js.Syntax.code("('WebSocket' in window || 'MozWebSocket' in window)");
		if (!supported) {
			cb.onConnect(false, "Websockets not available");
		}
		
		// if (isConnected) throw("WebSocket already connected");
	...
	{	
		// prevent to send more callbacks after closing
		ws.onopen = null;
		ws.onclose = null;
		ws.onerror = null;
		ws.onmessage = null;
